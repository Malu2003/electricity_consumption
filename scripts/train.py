...
data = data.drop(columns=['ages'], errors='ignore')  # Remove 'ages' (not part of model features)
data = data.fillna(data.median())  # Handle missing values

# Define Features (X) and Target Variable (Y)
X = data.drop(columns=['energy_consumption'])  # Ensure correct target column
...
SELECT u.consumer_number, u.family_members, u.working_members, u.ages, 
       ...
       STRING_AGG(DISTINCT a.appliance_name, ', ') AS appliance_names, 
       u.location
FROM users u
...
GROUP BY u.consumer_number, u.family_members, u.working_members, u.ages, 
         b.units_consumed, b.cost_per_unit, u.location;
"""
df = pd.read_sql_query(query, engine)

def process_ages(age_data):
    if isinstance(age_data, list):  # Check if it's a list
        return sum(age_data) / len(age_data)  # Take average age
    return float(age_data)  # Convert to float if it's a single number


df["ages"] = df["ages"].apply(process_ages)

# Merge appliance and electricity datasets
...
df = df.merge(appliance_data[['appliance_name', 'Power_Rating_W', 'Units_Consumed_Per_Hour_kWh']],
              left_on='appliance_names', right_on='appliance_name', how='left')
...
df = pd.get_dummies(df, columns=["appliance_names", "location"], drop_first=True)
