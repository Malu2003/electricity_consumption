...
SELECT u.consumer_number, u.family_members, u.working_members, u.ages, 
       ...
       u.location, a.usage_hours
FROM users u
LEFT JOIN bills b ON u.consumer_number = b.consumer_number
LEFT JOIN (
    SELECT consumer_number, appliance_name, SUM(usage_hours) AS usage_hours
    FROM appliances
    GROUP BY consumer_number, appliance_name
) a ON u.consumer_number = a.consumer_number;
"""
df = pd.read_sql_query(query, engine)

print("Data after fetching from SQL:")
print(df.head())
print("Raw data:")
print(df[['consumer_number', 'appliance_name', 'usage_hours']])
# Identify and fix unhashable data
...
print(df['usage_hours'].value_counts())

print("Usage hours distribution:")
print(df['usage_hours'].describe())
print("Usage hours value counts:")
print(df['usage_hours'].value_counts())

# Calculate total usage hours (SUM, not normalized)
total_usage_hours = df.pivot_table(values='usage_hours', index='consumer_number', aggfunc='sum').reset_index()
total_usage_hours.columns = ['consumer_number', 'total_usage_hours']
print("TOTALLL",total_usage_hours)
...
print("Data after calculating total_usage_hours:")
print(df.head())
...
df_final["effective_usage_hours"] = df_final["total_usage_hours"] * (
    1 - ((df_final["working_members"] * 8 / 24) / (df_final["family_members"].replace(0, 1) * 24)).fillna(0)
)
