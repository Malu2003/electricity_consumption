...
GROUP BY u.consumer_number, u.family_members, u.working_members, u.ages,a.usage_hours,
...
df["reduced_consumption"] = df["energy_consumption"] * (df["total_usage_hours"] / (df["total_usage_hours"] + 1e-6))
df["reduced_bill"] = df["reduced_consumption"] * df["cost_per_unit"]
df["reduced_carbon"] = df["reduced_consumption"] * 0.82
