...
print("before grouping",df[['consumer_number', 'appliance_name', 'usage_hours']].head(20))
#df = df.groupby(['consumer_number', 'appliance_name', 'location'], as_index=False).agg({'usage_hours': 'sum'})


total_usage_hours = df.groupby('consumer_number', as_index=False)['usage_hours'].sum()
print(total_usage_hours)
print("after groupinggggg",df[['consumer_number', 'usage_hours']].head(20))
...
print(df['consumer_number'].value_counts())
...
# -------------------- MODIFICATION: Ensuring Feature Consistency --------------------
# Ensure df has the same feature columns as training data (X_train)
expected_features = list(X_train.columns)
df_pred = df.copy()

for col in expected_features:
    if col not in df_pred.columns:
        df_pred[col] = 0  # Add missing columns

df_pred = df_pred[expected_features]

# -------------------- MODIFICATION: Correcting Reduced Consumption Calculation --------------------
# Merge necessary columns back after prediction
